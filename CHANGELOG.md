...
- **Accurate is now "retain up to x".** Retaining fewer dice is legal and sometimes better: a retained norm is locked, while rolling that dice can produce a critical success or a promotable norm. One die at 2+ with a spare norm→crit promotion is worth 3.33 damage rolled against a retained norm's 3.00, so the old always-retain-the-maximum behavior understated Accurate profiles. Both engines now evaluate every legal count and keep the best, with ties keeping the maximum. The Fight engine decides once per fighter profile (cached) rather than per simulated roll, because the real choice is made before rolling.
- All of these decisions are made in the dice step, before any damage is inflicted — matching the rule that dice are retained and modified up front, never adapted once shooting or a combat starts resolving.
- **Fight: lookaheads are now proper estimates.** `calcDieChoice` cloned both fighters *with* the live rng for its MaxDmgToEnemy/MinDmgToSelf comparison, so throwaway simulations pulled Feel No Pain and Saintly Relics rolls out of the real per-round streams — partially defeating the Common Random Numbers work from the previous release. Simply clearing the rng (as the two sibling lookaheads did) fixed that but introduced a second problem: `applyDmg` skips prevention entirely without an rng, so the estimate compared damage the defender would in fact have prevented.
- All three lookaheads now use `FighterState.asEstimate()`, which carries no rng and applies Feel No Pain and Saintly Relics as their **expected** values rather than rolling them. That keeps the real streams intact, still accounts for prevention, and — unlike rolling against a fixed seed — is smooth in its inputs, so a decision only flips when the underlying trade-off flips. The seeded-roll variant was tried first and measurably reintroduced the jitter CRN exists to remove: it broke the crit-damage monotonicity guard, raising a fighter's own death chance by 0.04pp when that fighter's own critical damage went up. Estimate wounds are fractional, which is fine as they are only ever compared.
- Markdown links between the in-app rules pages now navigate through the router instead of forcing a full page reload; external links keep their new-tab attributes. Added the new rules page to `sitemap.xml` and `llms.txt`.
- Corrected a changelog claim from the previous entry: the Severe/retained-norm ordering *preserves* the 2-crit result rather than newly producing it.
...
- Rules worded as *changing* a success rather than retaining one — **Severe** ("change one of your normal successes to a critical success") and the KT2021 **Waaagh** ploy — still apply to an already-retained dice, and now deliberately spend a retained norm first so the rolled ones stay available for the retain-style promotions that follow. This preference is what keeps the new restriction from over-applying: with a rolled norm plus a cover/Accurate norm, Severe and one NormsToCrits still reach 2 crits, exactly as before this change. (Taking the rolled norm instead would have left the retained one unpromotable and regressed that case to 1 crit + 1 norm.)
- The **FailsToNorms** input is unchanged: it is treated as a *change* effect, so its norms remain promotable. The in-app note now says so, and flags that Punishing — the retain-worded version — only fits rules sharing its conditions (needs a crit, off against Obscured).
...
- The Shock short-circuit no longer forces crit-first for a mixed crit+norm hand under the Strike/Max-Dmg/Min-Dmg strategies — it now defers to `preferredStrikeChoice`, so the norm-first parry-denial still applies and Shock's norm-cancel just lands on the later crit instead.
- The forced-strike exits (enemy has no successes, Shock) now route through a shared `strategyStrike` helper so the die order is chosen consistently. This fixes an order-sensitivity miss against an exhausted defender with **Just a Scratch**: crit-first wasted the crit on the first-strike negation, so the engine now strikes norm-first there to land the crit. The lethal-strike rule still strikes crit-first to land the killing blow.
...
- Fixed a latent bug exposed by these lookahead simulations: `handleDuelist` had no "already spent" flag, so every nested `resolveFight` (the strike-vs-parry lookahead and the new strike-order lookahead) re-granted Duelist's once-per-fight free parry, corrupting the estimate for Duelist fighters. Added `FighterState.hasDuelistParried` and gated the parry on it.
