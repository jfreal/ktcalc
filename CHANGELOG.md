...
- **Fight: the strike-vs-parry lookahead no longer consumes real RNG draws.** `calcDieChoice` cloned both fighters *with* the live rng for its MaxDmgToEnemy/MinDmgToSelf estimates, so throwaway simulations pulled Feel No Pain and Saintly Relics rolls out of the real per-round streams — partially defeating the Common Random Numbers work from the previous release. The clones now null their rng, matching `preferredStrikeChoice` and `calcParryForLastEnemySuccessThenKillEnemy`, with the same documented trade-off (rng-driven damage prevention isn't modeled inside the estimate).
