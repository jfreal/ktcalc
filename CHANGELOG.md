...
- The engine now uses **Common Random Numbers**: each simulation draws from independent, per-purpose streams (guy1 dice, guy2 dice, guy1 defense rolls, guy2 defense rolls), seeded from `(seed, simIndex, stream)`. Because streams are re-seeded per simulation, a change in one simulation's draw count no longer bleeds into later ones; and because each stream is seeded independently of the *other* fighter, two scenarios that differ in a single stat reuse identical dice everywhere the change doesn't reach. Comparisons now reflect the real effect instead of resampling noise, and the reported anomaly is monotone again (more of your own damage never raises your own death chance).
